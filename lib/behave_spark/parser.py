def to_dataframe(table):
    ...
    def to_name_type_tuple(heading):
        ...
        type_or_default = (name_with_type[1].strip()) if len(name_with_type) > 1 else 'str'
    ...
    def from_gherkin_table(table):
        return {
            ...
            for key, name_parser in zip(table.headings, [to_name_type_tuple(key) for key in table.headings])
        }
